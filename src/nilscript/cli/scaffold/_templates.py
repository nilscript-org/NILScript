...
def _gate_relation(client: SystemClient, env: dict[str, Any], field: str, model: str,
    ...
    try:
        ...
        if cands:
            return _refusal(env, "AMBIGUOUS", f"{{field}}: '{{value}}' matched several {{model}} — pick one of "
...
def _gate_option(env: dict[str, Any], field: str, options: list[dict[str, Any]],
                 value: Any) -> dict[str, Any] | None:
...
def create_app(client: SystemClient, emitter: EventEmitter, *, bearer: str | None) -> FastAPI:
    ...
    def propose(env: dict[str, Any] = Body(...), authorization: str | None = Header(None)) -> dict[str, Any]:
        ...
        if isinstance(verb_name, str) and verb_name.startswith("resource."):
            ...
            # by default the doctypes the curated verbs write, so a provisioned-but-undeclared model is
            # unexpressible (Guarantee 2 holds literally). Widen via an explicit allowlist if a flow needs it.
        ...
        # candidate options if not, so the agent picks the right one and re-proposes. to_native is a
        # best-effort here — an UNFILLED stub (NotImplementedError) skips the gate so PROPOSE stays a
        # clean dry-run; the real non-conformance surfaces at COMMIT, never as a 500 at PROPOSE.
        try:
    ...
    def query(env: dict[str, Any] = Body(...), authorization: str | None = Header(None)) -> dict[str, Any]:
        ...
        if body.get("verb") == "resource.read":
            ...
            # Reads obey the same skeleton bound — resource.read must not pull an undeclared model.
