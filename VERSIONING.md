...
- **Additive-in-place example (0.2.1):** the 7th performative, `ROLLBACK`, was added directly to the
  existing 0.1 wire dialect — no breaking major, no new schema namespace. The envelope const stays
  `"nil": "0.1"`; only the package version moved (`0.2.0 → 0.2.1`). Existing 0.1 messages, DSL
